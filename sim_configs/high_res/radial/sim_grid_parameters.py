...
# In microns
Y_HEIGHT = 28.16000000000001
DY_SIM = 1 / 20.
Z_HEIGHT = 28.16000000000001
DZ_SIM = 1 / 20.
T_LENGTH = 1000.
...
def compute_sim_grid(times, y_vals_output, z_vals_output):
    ...
    t_lo_index_output = np.argmin(np.abs(times + LASER_TIME_START))
    t_hi_index_output = np.argmin(np.abs(times - 120))
    ...
    '''
    t_lo_index_output = np.argmin(np.abs(times + 120))
    t_hi_index_output = np.argmin(np.abs(times - 120))
    t_lo_index_sim = 0
    t_hi_index_sim = np.argmin(np.abs(t_vals_sim_fs - (120 + 75)))
    if t_lo_index_sim >= 4:
        t_lo_index_sim -= 4
    else:
        t_lo_index_sim = 0

    if t_hi_index_sim <= len(t_vals_sim_fs) - 5:
        t_hi_index_sim += 4
    else:
        t_hi_index_sim = len(t_vals_sim_fs) - 1
    num_t_vals = t_hi_index_sim - t_lo_index_sim + 1'''
